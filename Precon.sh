...
if ! command -v figlet &> /dev/null; then
  ...
  color_code="\033[31m" # Red color code
    ...
    echo "Precon is a subdomain discovery bash script that discovers subdomains for websites by using various Tools."
...
if ! command -v figlet &> /dev/null; then
  ...
  color_code="\033[31m" # Red color code
    ...
    echo -e "${BLUE}Enumerating Subdomains for domain: $current_domain ${RESET}"
    log "${BLUE}Enumerating Subdomains for domain: $current_domain ${RESET}" 
    ...
    log "${BLUE}Precon completed. Output saved to: $current_domain ${RESET}" 
else
    echo -e "${WHITE}Precon Completed. Output saved to: $output_dir ${RESET}"
    log "${BLUE}Precon completed. Output saved to: $output_dir ${RESET}" 
fi
